...
void loop()
{
  ...
  if (clkState) {
    ...
    if (lastMilli != 0) {
      ...
      if (queueReady) {
        // on each clock in there will be at least one trig out, if there is one in the matrix.
        for (int i=0; i<noOfDrumSteps; i++) {
          state[i] = HIGH;
          milli[i] = millis();
          digitalWrite(pinOffset+(i), drums[drumProgram][i][currentPatternColumn]);
//          delay(1);
          digitalWrite(pinOffset+(i), LOW);
        }
